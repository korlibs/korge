...
/**
 * Previously registered tasks to manually close/promote OSSRH staging repositories.
 * With the new Sonatype Central Portal + vanniktech plugin, release is fully automatic
 * (automaticRelease = true). Tasks are kept as no-ops so CI scripts don't break.
 */
fun Project.configureMavenCentralRelease() {
    if (rootProject.tasks.findByName("releaseMavenCentral") == null) {
        rootProject.tasks.register("releaseMavenCentral") {
            ...
            it.doLast { logger.lifecycle("releaseMavenCentral: handled automatically by vanniktech/Central Portal.") }
        }
    }
    if (rootProject.tasks.findByName("checkReleasingMavenCentral") == null) {
        rootProject.tasks.register("checkReleasingMavenCentral") {
            it.group = "publishing"
            it.description = "No-op: staging check not required for Sonatype Central Portal."
