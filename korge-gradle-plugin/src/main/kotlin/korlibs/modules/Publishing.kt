...
fun Project.getCustomProp(name: String, default: String): String? {
    val props = if (rootProject.extra.has("props")) rootProject.extra["props"] as? Map<String, String>? else null
...
fun Project.configurePublishing(multiplatform: Boolean = true) {
    ...
    mavenPublishing.apply {
        ...
        pom {
            ...
            url.set(project.getCustomProp("project.scm.url", defaultGitUrl))
            licenses {
                license {
                    name.set(project.getCustomProp("project.license.name", "MIT") ?: "MIT")
                    url.set(
            ...
            developers {
                developer {
                    id.set(project.getCustomProp("project.author.id", "korge") ?: "korge")
                    name.set(project.getCustomProp("project.author.name", "KorGE Team") ?: "")
                    email.set(project.getCustomProp("project.author.email", "info@korge.org") ?: "")
                }
            }
            scm {
                url.set(project.getCustomProp("project.scm.url", defaultGitUrl))
