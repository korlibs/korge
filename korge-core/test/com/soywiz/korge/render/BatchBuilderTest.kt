	...
	fun simpleBatch() {
		...
		Assert.assertEquals(
			"""
				...
				draw(vertices=Buffer[0], indices=Buffer[1], program=Program(name=BatchBuilder2D, attributes=[a_Tex, a_Pos], uniforms=[u_ProjMat, u_Tex]), type=TRIANGLES, vertexLayout=VertexLayout[a_Pos, a_Tex], vertexCount=6, offset=0, blending=OVERLAY, uniforms={})
	...
	fun batch2() {
		...
		Assert.assertEquals(
			"""
				...
				draw(vertices=Buffer[0], indices=Buffer[1], program=Program(name=BatchBuilder2D, attributes=[a_Tex, a_Pos], uniforms=[u_ProjMat, u_Tex]), type=TRIANGLES, vertexLayout=VertexLayout[a_Pos, a_Tex], vertexCount=12, offset=0, blending=OVERLAY, uniforms={})
