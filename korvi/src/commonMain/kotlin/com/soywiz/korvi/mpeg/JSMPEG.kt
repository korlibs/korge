...
// Macroblock Layer
    ...
    fun copyMacroblock(motionH, motionV, sY, sCr, sCb) {
        var
        width, scan,
        H, V, oddH, oddV,
        src, dest, last;
    ...
    companion object {
        fun CopyBlockToDestination(block, dest, index, scan) {
        ...
        fun AddBlockToDestination(block, dest, index, scan) {
            for (var n = 0; n < 64; n += 8, index += scan+8) {
                dest[index + 0] += block[n + 0];
                dest[index + 1] += block[n + 1];
                dest[index + 2] += block[n + 2];
        ...
        fun CopyValueToDestination(value, dest, index, scan) {
            for (var n = 0; n < 64; n += 8, index += scan+8) {
                dest[index + 0] = value;
                dest[index + 1] = value;
                dest[index + 2] = value;
        ...
        fun AddValueToDestination(value, dest, index, scan) {
            for (var n = 0; n < 64; n += 8, index += scan+8) {
                dest[index + 0] += value;
                dest[index + 1] += value;
                dest[index + 2] += value;
