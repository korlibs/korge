...
class ParticleEmitter(val views: Views) {
	...
	suspend fun load(file: VfsFile): ParticleEmitter = this.apply {
		...
		for (item in particleXml.allChildrenNoComments) {
			...
			fun angle() = Math.toRadians(item.double("value"))
	...
	class Simulator(private val emitter: ParticleEmitter, var emitterPos: Point2d = Point2d()) {
		...
		fun init(particle: Particle): Particle {
			...
			particle.totalTime = Math.max(0.0, lifespan)
			...
			val startSize = Math.max(0.1, randomVariance(emitter.startSize, emitter.startSizeVariance))
			val endSize = Math.max(0.1, randomVariance(emitter.endSize, emitter.endSizeVariance))
		...
		fun advance(particle: Particle, _elapsedTime: Double) {
			...
			when (emitter.emitterType) {
				ParticleEmitter.Type.RADIAL -> {
					...
					particle.x = emitter.sourcePosition.x - Math.cos(particle.emitRotation) * particle.emitRadius
					particle.y = emitter.sourcePosition.y - Math.sin(particle.emitRotation) * particle.emitRadius
				}
				ParticleEmitter.Type.GRAVITY -> {
					val distanceX = particle.x - particle.startX
					val distanceY = particle.y - particle.startY
					val distanceScalar = Math.max(0.01, Math.sqrt(distanceX * distanceX + distanceY * distanceY))
