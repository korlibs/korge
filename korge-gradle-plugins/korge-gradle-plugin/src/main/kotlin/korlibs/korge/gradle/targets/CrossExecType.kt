...
enum class CrossExecType(val cname: String, val interp: String, val arch: String = "X64") {
    ...
    val interpCapital = interp.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() } + archNoX64
    val nameWithArch = "${cname}$arch"
    val nameWithArchCapital = nameWithArch.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
