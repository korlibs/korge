...
inline fun IntRange.toIntList(): IntArrayList = IntArrayList((this.endInclusive - this.start).coerceAtLeast(0)).also {
    for (v in this.start..this.endInclusive) it.add(v)
}
...
inline fun IntRange.mapInt(callback: (Int) -> Int): IntArrayList =
    IntArrayList((this.endInclusive - this.start).coerceAtLeast(0) / this.step + 1).also {
        for (v in this.start..this.endInclusive step this.step) it.add(callback(v))
    }

inline fun IntArrayList.mapInt(callback: (Int) -> Int): IntArrayList =
    IntArrayList(size).also { out -> this.fastForEach { out.add(callback(it)) } }

inline fun <T> Iterable<T>.mapInt(callback: (T) -> Int): IntArrayList =
    IntArrayList().also { for (v in this) it.add(callback(v)) }

inline fun <T> Iterable<T>.mapFloat(callback: (T) -> Float): FloatArrayList =
    FloatArrayList().also { for (v in this) it.add(callback(v)) }

inline fun <T> Iterable<T>.mapDouble(callback: (T) -> Double): DoubleArrayList =
    DoubleArrayList().also { for (v in this) it.add(callback(v)) }

inline fun BooleanArray.mapDouble(callback: (Boolean) -> Double): DoubleArray =
    DoubleArray(size).also { for (n in 0 until size) it[n] = callback(this[n]) }
...
inline fun BooleanArray.mapFloat(callback: (Boolean) -> Float): FloatArray =
    FloatArray(size).also { for (n in 0 until size) it[n] = callback(this[n]) }
...
inline fun BooleanArray.mapInt(callback: (Boolean) -> Int): IntArray =
    IntArray(size).also { for (n in 0 until size) it[n] = callback(this[n]) }
...
inline fun IntArrayList.filter(callback: (Int) -> Boolean): IntArrayList =
    IntArrayList().also { for (v in this) if (callback(v)) it.add(v) }

inline fun FloatArrayList.filter(callback: (Float) -> Boolean): FloatArrayList =
    FloatArrayList().also { for (v in this) if (callback(v)) it.add(v) }

inline fun DoubleArrayList.filter(callback: (Double) -> Boolean): DoubleArrayList =
    DoubleArrayList().also { for (v in this) if (callback(v)) it.add(v) }
