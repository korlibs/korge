...
fun <T : CharSequence> Array<T>.sortedRadix(
    start: Int = 0,
    end: Int = this.size,
    default: Char = '\u0000',
    transform: (Char) -> Char = { it }
): Array<T> = this.copyOf().also { it.sortRadix(start, end, default, transform) }

fun <T : CharSequence> Array<T>.sortRadix(
    start: Int = 0,
    end: Int = this.size,
    default: Char = '\u0000',
    transform: (Char) -> Char = { it }
...
fun IntArray.sortedArrayRadix(start: Int = 0, end: Int = this.size, bits: Int = 16): IntArray =
    this.copyOf().also { it.sortRadix(start, end, bits) }
...
inline fun <T> radixSortGeneric(
    start: Int,
    end: Int,
    stepStart: Int,
    stepEnd: Int,
    get: (index: Int) -> T,
    setTemp: (index: Int, v: T) -> Unit,
    flip: () -> Unit,
    getRadix: (index: Int, v: T) -> Int,
    noffsets: Int
) {
...
@PublishedApi
internal inline fun <T> _radixSortStep(
    start: Int,
    end: Int,
    get: (Int) -> T,
    setTemp: (Int, T) -> Unit,
