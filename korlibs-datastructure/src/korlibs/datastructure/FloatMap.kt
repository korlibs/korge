...
class FloatMap<T> private constructor(private var nbits: Int, private val loadFactor: Double) {
    ...
    inline fun fastValueForEachNullable(callback: (value: T?) -> Unit): Unit =
        map.fastValueForEachNullable { callback(it) }

    inline fun fastForEachNullable(callback: (key: Float, value: T?) -> Unit): Unit =
        map.fastForEachNullable { key, value -> callback(key.adapt(), value) }

    inline fun fastValueForEach(callback: (value: T) -> Unit): Unit = map.fastValueForEach { callback(it) }
    inline fun fastForEach(callback: (key: Float, value: T) -> Unit): Unit =
        map.fastForEach { key, value -> callback(key.adapt(), value) }
...
class FloatFloatMap private constructor(private var nbits: Int, private val loadFactor: Double) {
    ...
    inline fun getOrPut(key: Float, crossinline callback: () -> Float): Float =
        map.getOrPut(key.adapt()) { callback().adapt() }.adapt()
    ...
    inline fun fastForEach(callback: (key: Float, value: Float) -> Unit): Unit =
        map.fastForEach { key, value -> callback(key.adapt(), value.adapt()) }
...
class IntFloatMap private constructor(private var nbits: Int, private val loadFactor: Double) {
    ...
    inline fun getOrPut(key: Int, crossinline callback: () -> Float): Float =
        map.getOrPut(key) { callback().adapt() }.adapt()
    ...
    inline fun fastForEach(callback: (key: Int, value: Float) -> Unit): Unit =
        map.fastForEach { key, value -> callback(key, value.adapt()) }
...
class FloatIntMap private constructor(private var nbits: Int, private val loadFactor: Double) {
    ...
    inline fun fastForEach(callback: (key: Float, value: Int) -> Unit): Unit =
        map.fastForEach { key, value -> callback(key.adapt(), value) }
