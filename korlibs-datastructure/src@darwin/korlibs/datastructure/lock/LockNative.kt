...
actual class Lock actual constructor() : BaseLock {
    @PublishedApi
    internal var notified = AtomicReference(false)
    @PublishedApi
    internal var locked = AtomicReference(false)
    @PublishedApi
    internal var current: platform.posix.pthread_t? = null

    @PublishedApi
    internal fun lock(): Boolean {
    ...
    @PublishedApi
    internal fun unlock(initialThread: Boolean) {
        if (initialThread) {
            current = null
