...
class BitsTest {
	...
	fun name() {
		...
		assertEquals("0102030405060708", "%016X".format(a.readS64BE(0)))
		assertEquals("0807060504030201", "%016X".format(a.readS64LE(0)))

		assertEquals("01020304", "%08X".format(a.readS32BE(0)))
		assertEquals("04030201", "%08X".format(a.readS32LE(0)))

		assertEquals("010203", "%06X".format(a.readS24BE(0)))
		assertEquals("030201", "%06X".format(a.readS24LE(0)))

		assertEquals("0102", "%04X".format(a.readS16BE(0)))
		assertEquals("0201", "%04X".format(a.readS16LE(0)))

		assertEquals("01", "%02X".format(a.readS8(0)))

		val data = (0 until 128).map { ((it + 35363) * 104723).toByte() }.toByteArray()
		assertEquals(data.readS64BE(0), data.readS64LE(0).reverseBytes())
		assertEquals(data.readS32BE(0), data.readS32LE(0).reverseBytes())
		assertEquals(data.readU32BE(0).toInt(), data.readU32LE(0).toInt().reverseBytes())
		assertEquals(data.readS16BE(0).toShort(), data.readS16LE(0).toShort().reverseBytes())
		assertEquals(data.readU16BE(0).toShort(), data.readU16LE(0).toShort().reverseBytes())
	}
