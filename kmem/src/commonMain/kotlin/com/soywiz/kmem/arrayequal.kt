...
private inline fun _arrayequal(srcPos: Int, dstPos: Int, size: Int, cmp: (Int, Int) -> Boolean): Boolean {
    for (n in 0 until size) {
        if (!cmp(srcPos + n, dstPos + n)) return false
    }
    return true
}
...
fun arrayequal(src: Buffer, srcPos: Int, dst: Buffer, dstPos: Int, size: Int): Boolean = _arrayequal(srcPos, dstPos, size) { s, d -> src.getInt8(s) == dst.getInt8(d) }
