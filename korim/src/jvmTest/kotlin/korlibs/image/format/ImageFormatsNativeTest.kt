...
class ImageFormatsNativeTest {
  ...
  fun svg() = suspendTest {
    ...
    // assertEquals(
    //    """
    //        Couldn't read native image (fallback to non-native decoders):
    // java.lang.IllegalStateException: Can't read image using AWT
    //    """.trimIndent(),
    //    logs.joinToString("\n")
    // )
  ...
  @Test
  fun jpg() = suspendTest {
    val file = localVfs("C:\\Users\\kietm\\GitHub\\ImageCropper\\src\\commonMain\\resources\\converted.jpg")
    val bitmap = file.readNativeImage()
    assertEquals("AwtNativeImage(190, 190)", bitmap.toString())
    // awtShowImage(bitmap); Thread.sleep(10000L)
  }
}
