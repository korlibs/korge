...
class NativeDecodingTest {
    ...
    fun testReadAsumePremultiplied() = suspendTest {
        for (preferKotlinDecoder in listOf(false, true)) {
            ...
            val color1 = bitmap.getRgba(190, 50)
    ...
    fun testReadAll() = suspendTest {
        for (premultiplied in listOf(true, false, null)) {
            ...
            val x = 34; val y = 15
            assertNotEquals(0, image.getRgba(x, y).a)
            if (image.premultiplied) {
                assertEquals(colorPremult to true, image.getRgba(x, y) to image.premultiplied)
            } else {
                assertEquals(colorStraight to false, image.getRgba(x, y) to image.premultiplied)
            }
    ...
    private fun _testCreate(premultiplied: Boolean?) = suspendTest {
        ...
        assertEquals(colorStraight, bmp.getRgbaStraight(x, y))
    }
}
