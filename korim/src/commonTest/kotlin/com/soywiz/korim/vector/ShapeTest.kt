...
class ShapeTest {
    ...
    fun testShapeTransform() {
        val shape1 = buildShape {
            keepTransform {
                ...
                translate(200, 100)
                fill(
                    createLinearGradient(50, 50, 75, 120, transform = MMatrix(2, 0, 0, 2).immutable).addColorStop(0.0, Colors.RED)
    ...
    @Test
    fun testEmptyBounds() {
        fun createStrokeShape(block: VectorBuilder.() -> Unit): PolylineShape = PolylineShape(buildVectorPath { block() }, null, Colors.RED, MMatrix().immutable, StrokeInfo())
