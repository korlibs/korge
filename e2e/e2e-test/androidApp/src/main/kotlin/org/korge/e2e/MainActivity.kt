...
class MainActivity : KorgwActivity(config = GameWindowCreationConfig(msaa = 1, fullscreen = true)) {
    override suspend fun activityMain() {
        Korge(windowSize = Size2D(768, 512), backgroundColor = Colors["#2b2b2b"]) {
            ...
            try {
                ...
                for (case in cases) {
                    ...
                    delayFrame()
                    Log.d(TAG, "TAKING SCREENSHOT for ${case.name}")
                    try {
                        ...
                        e.printStackTrace()
                        Log.d(TAG, "SCREENSHOT NOT TAKEN (error ocurred)")
                    }
                }
            } catch (e: Throwable) {
                e.printStackTrace()
                ...
                Log.d(TAG, "CLOSING WINDOW: exitCode=$exitCode, exceptions=${exceptions.size}")
