...
class WindowsGameWindow : EventLoopGameWindow() {
    ...
    fun keyUpdate(keyCode: Int, down: Boolean) {
        keyEvent(keyCode, if (down) korlibs.event.KeyEvent.Type.DOWN else korlibs.event.KeyEvent.Type.UP)
    }

    fun keyType(character: Int) {
        keyEvent(character, korlibs.event.KeyEvent.Type.TYPE)
    }
...
fun WndProc(hWnd: HWND?, message: UINT, wParam: WPARAM, lParam: LPARAM): LRESULT {
    ...
    when (message) {
        ...
        _WM_MOUSEWHEEL, _WM_MOUSEHWHEEL -> {
            ...
            val type = korlibs.event.MouseEvent.Type.SCROLL
...
fun mouseMove(x: Int, y: Int, wParam: Int) {
    ...
    windowsGameWindow.mouseEvent(
        if (anyButton) korlibs.event.MouseEvent.Type.DRAG else korlibs.event.MouseEvent.Type.MOVE,
...
fun mouseButton(button: Int, down: Boolean, wParam: Int) {
    //buttons[button] = down
    if (down) {
        windowsGameWindow.mouseEvent(korlibs.event.MouseEvent.Type.DOWN, mouseX, mouseY, button, wParam)
    } else {
        windowsGameWindow.mouseEvent(korlibs.event.MouseEvent.Type.UP, mouseX, mouseY, button, wParam)
        windowsGameWindow.mouseEvent(
            korlibs.event.MouseEvent.Type.CLICK,
