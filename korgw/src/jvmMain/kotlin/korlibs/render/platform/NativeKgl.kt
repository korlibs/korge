...
open class NativeKgl constructor(private val gl: INativeGL) : KmlGlWithExtensions() {
    ...
    override fun uniformMatrix2fv(location: Int, count: Int, transpose: Boolean, value: Buffer): Unit = gl.glUniformMatrix2fv(location, count, transpose.toByte(), value.directFloatBuffer.slice(0, 4 * count))
    override fun uniformMatrix3fv(location: Int, count: Int, transpose: Boolean, value: Buffer): Unit = gl.glUniformMatrix3fv(location, count, transpose.toByte(), value.directFloatBuffer.slice(0, 9 * count))
    override fun uniformMatrix4fv(location: Int, count: Int, transpose: Boolean, value: Buffer): Unit = gl.glUniformMatrix4fv(location, count, transpose.toByte(), value.directFloatBuffer.slice(0, 16 * count))
