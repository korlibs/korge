...
class GlutGameWindow : GameWindow(), DialogInterface by ZenityDialogs {
    ...
    fun mouseEvent(etype: korlibs.event.MouseEvent.Type, ex: Int, ey: Int, ebutton: Int) {
        dispatch(mouseEvent {
            this.type = etype
            this.x = ex
            this.y = ey
...
val glutGameWindow = korlibs.render.GlutGameWindow()
...
fun glutMouseMove(x: Int, y: Int) {
    glutGameWindow.mouseEvent(korlibs.event.MouseEvent.Type.MOVE, x, y, 0)
}
...
fun glutMouse(button: Int, state: Int, x: Int, y: Int) {
    ...
    if (up) {
        glutGameWindow.mouseEvent(korlibs.event.MouseEvent.Type.CLICK, x, y, button)
    }
}
