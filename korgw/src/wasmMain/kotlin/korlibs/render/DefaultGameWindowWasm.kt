...
private abstract external class WindowEx : Window {
    var gl: JsReference<KmlGl>
    var ag: JsReference<AGOpengl>
    var canvas: HTMLCanvasElement
    var korgwShowUnsupportedKeys: Boolean?
}

public abstract external class TouchEx : org.w3c.dom.Touch {
    var force: JsNumber?
}
...
open class BrowserCanvasJsGameWindow(
        ...
        val key = when (me.key) {
            ...
            "\"" -> Key.QUOTE
                ...
                "Tab" -> Key.TAB
                    ...
                    if (window.unsafeCast<WindowEx>().korgwShowUnsupportedKeys != null) {
    ...
    private fun touchEvent(e: TouchEvent, type: korlibs.event.TouchEvent.Type) {
        ...
        dispatch(touchBuilder.frame(TouchBuilder.Mode.JS, type) {
            for (n in 0 until e.touches.length) {
                val touch = e.touches.item(n)?.unsafeCast<TouchEx>() ?: continue
                ...
                touch(
                    ...
                    force = touch.force.jsDyn.toFloat(1f),
    ...
    init {
        ...
        canvas.ondragenter = { dispatchDropfileEvent(DropFileEvent.Type.START, null); null }
        canvas.ondragexit = { dispatchDropfileEvent(DropFileEvent.Type.END, null); null }
        canvas.ondragleave = { dispatchDropfileEvent(DropFileEvent.Type.END, null); null }
        canvas.ondragover = { it.preventDefault(); null }
        canvas.ondragstart = { dispatchDropfileEvent(DropFileEvent.Type.START, null); null }
        canvas.ondragend = { dispatchDropfileEvent(DropFileEvent.Type.END, null); null }
        canvas.ondrop = {
            it.preventDefault()
