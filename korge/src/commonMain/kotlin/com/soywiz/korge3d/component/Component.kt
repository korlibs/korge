...
/**
 * An interface that allows to control the behaviour of a [View3D] after some events.
...
interface ResizeComponent : Component {
    ...
    companion object {
        operator fun invoke(view: View3D, block: Views.(width: Int, height: Int) -> Unit): ResizeComponent =
            object : ResizeComponent {
                override val view: View3D = view
...
fun <T : View3D> T.onStageResized(firstTrigger: Boolean = true, block: Views.(width: Int, height: Int) -> Unit): T = this.apply {
    if (firstTrigger) {
