...
class KmlGlWasmCanvas(val canvas: HTMLCanvasElement, val glOpts: JsAny) : KmlGl() {
    ...
    override fun bufferData(target: Int, size: Int, data: Buffer, usage: Int): Unit = gl.bufferData(target, data._slice(0, size).arrayUByte, usage)
    override fun bufferSubData(target: Int, offset: Int, size: Int, data: Buffer): Unit = gl.bufferSubData(target, offset, data._slice(0, size).arrayUByte)
