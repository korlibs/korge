...
import com.soywiz.korio.vfs.LogVfs
import com.soywiz.korio.vfs.VfsFile

abstract class ResourceProcessor(vararg extensions: String) : Services.Impl() {
	abstract val version: Int
	abstract val outputExtension: String

	val extensionLCs = extensions.toList().map(String::toLowerCase)
	suspend abstract protected fun processInternal(inputFile: VfsFile, outputFile: VfsFile): Unit
	...
	suspend private fun _checkAndProcess(file: VfsFile, outputFolder: VfsFile, doProcess: Boolean): Boolean {
		val inputFile = file
	...
	companion object {
		val processorsByExtension: Map<String, ResourceProcessor> by lazy {
			try {
				Services.loadList(ResourceProcessor::class.java).flatMap { processor -> processor.extensionLCs.map { it to processor } }.toMap()
			} catch (e: Throwable) {
				e.printStackTrace()
