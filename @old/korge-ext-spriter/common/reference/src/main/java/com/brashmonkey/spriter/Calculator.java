...
public class Calculator {
	...
	public final static float PI = (float)PI;
	 ...
	 */
	public static float sqrt(float x){ return (float)sqrt(x); }
	 ...
	 */
	public static float acos(float x){ return (float)acos(x); }
	...
	static private class Sin {
		...
		static {
			for (int i = 0; i < SIN_COUNT; i++)
				table[i] = (float)sin((i + 0.5f) / SIN_COUNT * radFull);
			for (int i = 0; i < 360; i += 90)
				table[(int)(i * degToIndex) & SIN_MASK] = (float)sin(i * degreesToRadians);
		}
	}
