...
const val MINIMP3_MAX_SAMPLES_PER_FRAME = (1152*2)

class mp3dec_frame_info_t {
...
class mp3dec_t {
    val mdct_overlap = Array(2) { FloatArray(9 * 32) }
    val qmf_state = FloatArray(15*2*32);
    var reserv: Int = 0
    var free_format_bytes: Int = 0
    val header = UByteArray(4)
    val reserv_buf = UByteArray(511)
}

typealias mp3d_sample_t = Short

const val MAX_FREE_FORMAT_FRAME_SIZE  = 2304    /* more than ISO spec's */
const val MAX_FRAME_SYNC_MATCHES      = 10

const val MAX_L3_FRAME_PAYLOAD_BYTES  = MAX_FREE_FORMAT_FRAME_SIZE /* MUST be >= 320000/8/32000*1152 = 1440 */
...
fun HDR_IS_MONO(h)              = (((h[3])  and  0xC0) == 0xC0)
fun HDR_IS_MS_STEREO(h)         = (((h[3])  and  0xE0) == 0x60)
fun HDR_IS_FREE_FORMAT(h)       = (((h[2])  and  0xF0) == 0)
fun HDR_IS_CRC(h)               = (!((h[1])  and  1))
fun HDR_TEST_PADDING(h)         = ((h[2])  and  0x2)
fun HDR_TEST_MPEG1(h)           = ((h[1])  and  0x8)
fun HDR_TEST_NOT_MPEG25(h)      = ((h[1])  and  0x10)
fun HDR_TEST_I_STEREO(h)        = ((h[3])  and  0x10)
fun HDR_TEST_MS_STEREO(h)       = ((h[3])  and  0x20)
fun HDR_GET_STEREO_MODE(h)      = (((h[3])  shr  6)  and  3)
fun HDR_GET_STEREO_MODE_EXT(h)  = (((h[3])  shr  4)  and  3)
fun HDR_GET_LAYER(h)            = (((h[1])  shr  1)  and  3)
fun HDR_GET_BITRATE(h)          = ((h[2])  shr  4)
fun HDR_GET_SAMPLE_RATE(h)      = (((h[2])  shr  2)  and  3)
fun HDR_GET_MY_SAMPLE_RATE(h)   = (HDR_GET_SAMPLE_RATE(h) + (((h[1]  shr  3)  and  1) + ((h[1]  shr  4)  and  1))*3)
fun HDR_IS_FRAME_576(h)         = ((h[1]  and  14) == 2)
fun HDR_IS_LAYER_1(h)           = ((h[1]  and  6) == 6)

const val BITS_DEQUANTIZER_OUT        = -1
const val MAX_SCF                     = (255 + BITS_DEQUANTIZER_OUT*4 - 210)
const val MAX_SCFI                    = ((MAX_SCF + 3)  and  ~3)

fun MINIMP3_MIN(a, b)           = ((a) > (b) ? (b) : (a))
fun MINIMP3_MAX(a, b)           = ((a) < (b) ? (b) : (a))

class UBytePtr(val data: UByteArray, var offset: Int) {
    fun get() = data[offset]
    operator fun get(index: Int) = data[offset + index]
    operator fun set(index: Int, value: UByte) = run { data[index] = value }
    operator fun plus(offset: Int) = UBytePtr(this.data, this.offset + offset)
    operator fun inc(): UBytePtr {
        return UBytePtr(data, offset++)
    }
}
...
fun bs_init(bs: bs_t, data: UBytePtr, bytes: Int)
{
    bs.buf   = data;
    bs.pos   = 0;
    bs.limit = bytes*8;
...
fun get_bits(bs: bs_t, n: Int): UInt
{
    var next: UInt
    var cache = 0u
    var s = bs.pos and 7;
    var shl = n + s;
    var p = bs.buf + (bs.pos  shr  3);
    if ((bs.pos += n) > bs.limit) return 0u;
    next = (p++).get().toUInt()  and  (255  shr  s);
    while ((shl -= 8) > 0)
    {
        cache  or = next << shl;
        next = *p++;
...
fun hdr_valid(h: UBytePtr): Boolean  {
    return h[0] == 0xff.toUByte() && ((h[1]  and  0xF0.toUByte()) == 0xf0.toUByte() || (h[1]  and  0xFE.toUByte()) == 0xe2.toUByte()) &&
    (HDR_GET_LAYER(h) != 0) && (HDR_GET_BITRATE(h) != 15) && (HDR_GET_SAMPLE_RATE(h) != 3);
}

fun hdr_compare(h1: UBytePtr, h2: UBytePtr): Boolean
{
    return hdr_valid(h2) && ((h1[1]  xor   h2[1])  and  0xFE) == 0 && ((h1[2]  xor   h2[2])  and  0x0C) == 0 && !(HDR_IS_FREE_FORMAT(h1)  xor   HDR_IS_FREE_FORMAT(h2));
}
...
static unsigned hdr_sample_rate_hz(const uint8_t *h)
{
    ...
    return g_hz[HDR_GET_SAMPLE_RATE(h)]  shr  (int)!HDR_TEST_MPEG1(h)  shr  (int)!HDR_TEST_NOT_MPEG25(h);
}

static unsigned hdr_frame_samples(const uint8_t *h)
{
    return HDR_IS_LAYER_1(h) ? 384 : (1152  shr  (int)HDR_IS_FRAME_576(h));
}
...
static const L12_subband_alloc_t *L12_subband_alloc_table(const uint8_t *hdr, L12_scale_info *sci)
{
    ...
    } else
    {
        ...
        unsigned kbps = hdr_bitrate_kbps(hdr)  shr  (int)(mode != MODE_MONO);
        if (!kbps) /* free-format */
        {
            kbps = 192;
        }
...
static void L12_read_scalefactors(bs_t *bs, uint8_t *pba, uint8_t *scfcod, int bands, float *scf)
{
    ...
    for (i = 0; i < bands; i++)
    {
        float s = 0;
        int ba = *pba++;
        int mask = ba ? 4 + ((19  shr  scfcod[i])  and  3) : 0;
        for (m = 4; m; m  shr = 1)
        {
            if (mask  and  m)
            {
                int b = get_bits(bs, 6);
                s = g_deq_L12[ba*3 - 6 + b % 3]*(1 << 21  shr  b/3);
            }
            *scf++ = s;
        }
    }
        ...
        for (i = 0; i < 2*sci.total_bands; i++)
        {
            ...
            if (ba != 0)
            {
                ...
                } else
                {
                    ...
                    unsigned code = get_bits(bs, mod + 2 - (mod  shr  3));  /* 5, 7, 10 */
...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    do
    {
        ...
        gr.table_select[0] = (uint8_t)(tables  shr  10);
        gr.table_select[1] = (uint8_t)((tables  shr  5)  and  31);
        gr.table_select[2] = (uint8_t)((tables)  and  31);
        ...
        gr.scfsi = (uint8_t)((scfsi  shr  12)  and  15);
        scfsi <<= 4;
        gr++;
    } while(--gr_count);
...
static float L3_ldexp_q2(float y, int exp_q2)
{
    ...
    do
    {
        ...
        y *= g_expfrac[e  and  3]*(1 << 30  shr  (e  shr  2));
    } while ((exp_q2 -= e) > 0);
    return y;
}
...
static void L3_decode_scalefactors(const uint8_t *hdr, uint8_t *ist_pos, bs_t *bs, const L3_gr_info_t *gr, float *scf, int ch)
{
    ...
    if (HDR_TEST_MPEG1(hdr))
    {
        ...
        scf_size[1] = scf_size[0] = (uint8_t)(part  shr  2);
        scf_size[3] = scf_size[2] = (uint8_t)(part  and  3);
    } else
    {
        ...
        sfc = gr.scalefac_compress  shr  ist;
...
static float L3_pow_43(int x)
{
    ...
    sign = 2*x  and  64;
    frac = (float)((x  and  63) - sign) / ((x  and  ~63) + sign);
    return g_pow43[16 + ((x + sign)  shr  6)]*(1.f + frac*((4.f/3) + frac*(2.f/9)))*mult;
}
...
static void L3_huffman(float *dst, bs_t *bs, const L3_gr_info_t *gr_info, const float *scf, int layer3gr_limit)
{
    ...
    #define PEEK_BITS(n)  (bs_cache  shr  (32 - n))
    #define FLUSH_BITS(n) { bs_cache <<= (n); bs_sh += (n); }
    #define CHECK_BITS    while (bs_sh >= 0) { bs_cache  or = (uint32_t)*bs_next_ptr++ << bs_sh; bs_sh -= 8; }
    ...
    uint32_t bs_cache = (((bs_next_ptr[0]*256u + bs_next_ptr[1])*256u + bs_next_ptr[2])*256u + bs_next_ptr[3]) << (bs.pos  and  7);
    int pairs_to_decode, np, bs_sh = (bs.pos  and  7) - 8;
    bs_next_ptr += 4;
        ...
        do
        {
            ...
            do
            {
                ...
                while (leaf < 0)
                {
                    FLUSH_BITS(w);
                    w = leaf  and  7;
                    leaf = codebook[PEEK_BITS(w) - (leaf  shr  3)];
                }
                FLUSH_BITS(leaf  shr  8);

                for (j = 0; j < 2; j++, dst++, leaf  shr = 4)
                {
                    int lsb = leaf  and  0x0F;
                    if (lsb == 15 && linbits)
                    {
                    ...
                    {
                        *dst = g_pow43[16 + lsb - 16*(bs_cache  shr  31)]*one;
                    }
                    FLUSH_BITS(lsb ? 1 : 0);
                }
                CHECK_BITS;
    ...
    for (np = 1 - big_val_cnt;; dst += 4)
    {
        ...
        if (!(leaf  and  8))
        {
            leaf = codebook_count1[(leaf  shr  3) + (bs_cache << 4  shr  (32 - (leaf  and  3)))];
        }
        FLUSH_BITS(leaf  and  7);
        if (BSPOS > layer3gr_limit)
        {
            break;
        }
        #define RELOAD_SCALEFACTOR  if (!--np) { np = *sfb++/2; if (!np) break; one = *scf++; }
        #define DEQ_COUNT1(s) if (leaf  and  (128  shr  s)) { dst[s] = ((int32_t)bs_cache < 0) ? -one : one; FLUSH_BITS(1) }
        RELOAD_SCALEFACTOR;
        DEQ_COUNT1(0);
        DEQ_COUNT1(1);
        RELOAD_SCALEFACTOR;
...
static void L3_stereo_process(float *left, const uint8_t *ist_pos, const uint8_t *sfb, const uint8_t *hdr, int max_band[3], int mpeg2_sh)
{
    ...
    for (i = 0; sfb[i]; i++)
    {
        ...
        if ((int)i > max_band[i % 3] && ipos < max_pos)
        {
            ...
            {
                kl = 1;
                kr = L3_ldexp_q2(1, (ipos + 1)  shr  1 << mpeg2_sh);
                if (ipos  and  1)
                {
                    kl = kr;
                    kr = 1;
...
static void L3_intensity_stereo(float *left, uint8_t *ist_pos, const L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    L3_stereo_process(left, ist_pos, gr.sfbtab, hdr, max_band, gr[1].scalefac_compress  and  1);
}
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs, mp3dec_scratch_t *s, int main_data_begin)
{
    ...
    bs_init( and s.bs, s.maindata, bytes_have + frame_bytes);
...
static void L3_decode(mp3dec_t *h, mp3dec_scratch_t *s, L3_gr_info_t *gr_info, int nch)
{
    ...
    for (ch = 0; ch < nch; ch++)
    {
        ...
        L3_decode_scalefactors(h.header, s.ist_pos[ch],  and s.bs, gr_info + ch, s.scf, ch);
        L3_huffman(s.grbuf[ch],  and s.bs, gr_info + ch, s.scf, layer3gr_limit);
    }
...
static void mp3d_DCT_II(float *grbuf, int n)
{
    ...
    if (have_simd()) for (; k < n; k += 4)
    {
        ...
        {
            f4 x0 = VLD( and y[i*18]);
            f4 x1 = VLD( and y[(15 - i)*18]);
            f4 x2 = VLD( and y[(16 + i)*18]);
            f4 x3 = VLD( and y[(31 - i)*18]);
            f4 t0 = VADD(x0, x3);
            f4 t1 = VADD(x1, x2);
        ...
        {
            #if HAVE_SSE
            #define VSAVE2(i, v) _mm_storel_pi((__m64 *)(void*) and y[i*18], v)
            #else /* HAVE_SSE */
            #define VSAVE2(i, v) vst1_f32((float32_t *) and y[i*18],  vget_low_f32(v))
            #endif /* HAVE_SSE */
        ...
        {
            #define VSAVE4(i, v) VSTORE( and y[i*18], v)
            for (i = 0; i < 7; i++, y += 4*18)
            {
                f4 s = VADD(t[3][i], t[3][i + 1]);
                VSAVE4(0, t[0][i]);
...
fun mp3d_synth(xl: FloatPtr, dstl: mp3d_sample_t, nch: Int, lins: FloatPtr)
{
    int i;
    float *xr = xl + 576*(nch - 1);
    mp3d_sample_t *dstr = dstl + (nch - 1);

    val g_win = floatArrayOf(
    ...
    {
        #define VLOAD(k) f4 w0 = VSET(*w++); f4 w1 = VSET(*w++); f4 vz = VLD( and zlin[4*i - 64*k]); f4 vy = VLD( and zlin[4*i - 64*(15 - k)]);
    ...
    for (i = 14; i >= 0; i--)
    {
        #define LOAD(k) float w0 = *w++; float w1 = *w++; float *vz =  and zlin[4*i - k*64]; float *vy =  and zlin[4*i - (15 - k)*64];
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    if (!frame_size)
    {
        ...
        i = mp3d_find_frame(mp3, mp3_bytes,  and dec.free_format_bytes,  and frame_size);
    ...
    if (info.layer == 3)
    {
        ...
        success = L3_restore_reservoir(dec, bs_frame,  and scratch, main_data_begin);
        if (success)
        {
            for (igr = 0; igr < (HDR_TEST_MPEG1(hdr) ? 2 : 1); igr++, pcm += 576*info.channels)
            {
                memset(scratch.grbuf[0], 0, 576*2*sizeof(float));
                L3_decode(dec,  and scratch, scratch.gr_info + igr*info.channels, info.channels);
                mp3d_synth_granule(dec.qmf_state, scratch.grbuf[0], 18, info.channels, pcm, scratch.syn[0]);
            }
        }
        L3_save_reservoir(dec,  and scratch);
    } else
    {
        ...
        {
            if (12 == (i += L12_dequantize_granule(scratch.grbuf[0] + i, bs_frame, sci, info.layer  or  1)))
            {
                i = 0;
...
void mp3dec_f32_to_s16(const float *in, int16_t *out, int num_samples)
{
    if(num_samples > 0)
    {
        int i = 0;
        #if HAVE_SIMD
        int aligned_count = num_samples  and  ~7;

        for(;i < aligned_count;i+=8)
        {
            static const f4 g_scale = { 32768.0f, 32768.0f, 32768.0f, 32768.0f };
            f4 a = VMUL(VLD( and in[i  ]), g_scale);
            f4 b = VMUL(VLD( and in[i+4]), g_scale);
            #if HAVE_SSE
            static const f4 g_max = { 32767.0f, 32767.0f, 32767.0f, 32767.0f };
            static const f4 g_min = { -32768.0f, -32768.0f, -32768.0f, -32768.0f };
            __m128i pcm8 = _mm_packs_epi32(_mm_cvtps_epi32(_mm_max_ps(_mm_min_ps(a, g_max), g_min)),
