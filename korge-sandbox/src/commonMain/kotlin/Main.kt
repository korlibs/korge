...
suspend fun Stage.demoSelector(default: Demo, all: List<Demo>) {
    val container = sceneContainer(width = width, height = height - 48f) { }.xy(0, 48)
    ...
    uiHorizontalStack(padding = 8f) {
        ...
        comboBox = uiComboBox<Demo>(width = 200f, items = (listOf(default) + all).distinctBy { it.name }.sortedBy { it.name }) {
        ...
        uiCheckBox(width = 200f, text = "forceRenderEveryFrame", checked = views.forceRenderEveryFrame) {
            onChange { views.forceRenderEveryFrame = it.checked }
        }
        uiCheckBox(width = 150f, text = "toggleDebug", checked = views.debugViews) {
