...
class CCDResolver(player: Player) : IKResolver(player) {
	...
	public override fun resolve(x: Float, y: Float, chainLength: Int, effectorRef: BoneRef) {
		...
		var yy = unmappedffector.position.y + Math.sin(Math.toRadians(unmappedffector.angle.toDouble())).toFloat() * width
		...
		if (Math.signum(player.root.scale.x) == -1f) effector.angle += 180f
		...
		while (i < chainLength && parentRef != null) {
			...
			if (parentRef != null && i < chainLength - 1) {
				...
				parent = null
			player.unmapObjects(null)
			xx = unmappedffector.position.x + Math.cos(Math.toRadians(unmappedffector.angle.toDouble())).toFloat() * width
			yy = unmappedffector.position.y + Math.sin(Math.toRadians(unmappedffector.angle.toDouble())).toFloat() * width
			i++
		}
	}
