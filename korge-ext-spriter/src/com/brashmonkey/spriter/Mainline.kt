...
class Mainline(keys: Int) {
	...
	class Key(val id: Int, val time: Int, val curve: Curve, boneRefs: Int, objectRefs: Int) {
		...
		override fun toString(): String {
			var toReturn = javaClass.simpleName + "|[id:" + id + ", time: " + time + ", curve: [" + curve + "]"
			for (ref in boneRefs)
				toReturn += "\n" + ref
		...
		open class BoneRef(@JvmField val id: Int, @JvmField val timeline: Int, @JvmField val key: Int, @JvmField val parent: BoneRef?) {
			...
			override fun toString(): String {
				...
				return javaClass.simpleName + "|id: " + id + ", parent:" + parentId + ", timeline: " + timeline + ", key: " + key
			}
		}
		...
		class ObjectRef(id: Int, timeline: Int, key: Int, parent: BoneRef?, val zIndex: Int) : BoneRef(id, timeline, key, parent), Comparable<ObjectRef> {
			...
			override fun compareTo(o: ObjectRef): Int {
				return Math.signum((zIndex - o.zIndex).toFloat()).toInt()
			}
		}
