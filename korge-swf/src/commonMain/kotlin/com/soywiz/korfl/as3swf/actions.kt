...
class ActionExecutionContext(
	...
	var endLabel: String? = if (actions.firstOrNull { it is IActionBranch && it.branchIndex == -1 } != null) "L" + (labelCount + 1) else null
}
...
class ActionGetURL2(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	...
	override fun toString(indent: Int): String =
		"[ActionGetURL2] SendVarsMethod: $sendVarsMethod (${sendVarsMethodToString()}), Reserved: $reserved, LoadTargetFlag: $loadTargetFlag, LoadVariablesFlag: $loadVariablesFlag"

	override fun toBytecode(indent: Int, context: ActionExecutionContext): String = toBytecodeLabel(indent) +
	...
	fun sendVarsMethodToString(): String = when (sendVarsMethod) {
		0 -> "None"
		1 -> "GET"
		2 -> "POST"
		else -> error("sendVarsMethod is only defined for values of 0, 1, and 2.")
	}
}
...
class ActionGotoFrame2(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	...
	override fun toString(indent: Int): String {
		var str = "[ActionGotoFrame2] PlayFlag: $playFlag, SceneBiasFlag: $sceneBiasFlag"
...
class ActionIf(code: Int, length: Int, pos: Int) : Action(code, length, pos), IActionBranch {
	...
	override fun toString(indent: Int): String {
		val bi = "[" + when {
			branchIndex >= 0 -> branchIndex.toString()
			branchIndex == -1 -> "EOB"
			else -> "???"
		} + "]"
	...
	override fun toBytecode(indent: Int, context: ActionExecutionContext): String {
		val ls = when {
			branchIndex >= 0 -> context.actions[branchIndex]?.lbl
			branchIndex == -1 -> "L" + (context.labelCount + 1)
			else -> "ILLEGAL BRANCH"
		}
...
class ActionPush(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	var values = ArrayList<SWFActionValue>()
...
class ActionConstantPool(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	...
	override fun toString(indent: Int): String {
		var str = "[ActionConstantPool] Values: " + constants.size
...
class ActionStackSwap(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	override fun toString(indent: Int): String = "[ActionStackSwap]"
	override fun toBytecode(indent: Int, context: ActionExecutionContext): String = toBytecodeLabel(indent) + "stackSwap"
}
...
class ActionDefineFunction2(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	var functionName: String? = null
	var functionParams = ArrayList<SWFRegisterParam>()
	var functionBody = ArrayList<IAction>()
	private var labelCount: Int = 0
...
open class ActionTry(code: Int, length: Int, pos: Int) : Action(code, length, pos), IAction {
	...
	private var labelCountTry: Int = 0
	private var labelCountCatch: Int = 0
	private var labelCountFinally: Int = 0
